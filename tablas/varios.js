...
var sValorCapturado="", sErr="";
		...
		if (oTbl.rows.length>1){
			for (i=1; i<=10; i++){
				...
				if (isNaN(sValorCapturado))
					...
					{
						nCalif++;
						oTbl.rows[i].style.backgroundColor="rgb(12, 185, 55)";
						oTbl.rows[i].style.color="rgb(255, 255, 255)";
					}
						else{
							oTbl.rows[i].style.backgroundColor="rgb(190, 22, 22)";
							oTbl.rows[i].style.color="rgb(255, 255, 255)";
						}
	...
	if (sErr == ""){
		...
		var x = document.innerHTML="Calificación: "+nCalif;
		document.getElementById("calificacion").innerHTML = x;
	}
	else
		alert(sErr);
}
